var messages = {
  ...
  "no_speech": {
    ...
    class: 'alert-danger'},
  "no_microphone": {
    msg: 'Tidak ada microphone yang terdeteks <a href="//support.google.com/chrome/answer/2693767" target="_blank">microphone settings</a> .',
    class: 'alert-danger'},
  "allow": {
...
$('#summarize').on('click', function(e){
    ...
    $.ajax({
        ...
        success: function(result){
            console.log(result)
            $('#status').text('Result')
            $('#result_summarize').show()
            $('#data').text(result.results)
        },
        error: function(err){
            console.log(err)
        }
